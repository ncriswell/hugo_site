...
mu_out <- F_get_weekly_matchup(season = 2019, week = 1:5)
mu_view0 <- mu_out$mu_view0

stat_out <- F_get_weekly_player_stats(season = 2019, week = 1:5)
...
Max lost his perfect record this week. Nate and Otto kept theirs. 
...
Now that I am two weeks into my downturn, I feel like it is OK for me to worry. Things aren't looking good. 

I don't know if I'm crazy about this view. And that isn't just because my blue line is moving the wrong way. It all seems sort of bunched up. We could allow each plot to sort of have a floating y-axis and then the movement would be a little more obvious but the relative scales will be harder to compare between Owners. But, hey, space on a website it cheap nd it requires changing literally two characters in the code, so let's go for it. 
...
This makes it a little easier to see the bounciness of people like Scott, Tony, Jon, James and the consistency (for ~~better or~~ worse) of Sam and Otto. 
...
The higher the bump, the more likely you are to find a weekly point total for that owner at that point on the x-axis. People who have large variation in their point totals will see a broad-ish and shallow-ish distribution. People who are consistent will have more concentrated, single bumps. So again, Otto and Charlie are pretty consistent on their Points For. Most of us have a reasonable variation on Points Against, but we see people like Zach and Nate have their Points Against shifted to the higher end, indicating they take lots of punishment. People like Dave, Max, Jon, and me have our Points Against shifted to the lower end. 
...
As per usual for Zach, he outperformed most other players but still lost. This also happened to Max and Nate, but that is more of a new experience for them. Boy, Joe really came by his loss honestly last week, didn't he? Worst score by a lot from the commish. What I'm saying is that he got absolutely killed. 
...
This graph continues to tell the story of Zach's season so far. He scores a ton but has the worst matchup luck imaginable. I am slipping in the round robin rankings as my losing streak continues. It looks to me like Zach, Jon and Dave are the teams to beat right now. Sam shows extreme consistency relative to the rest of the league and Scott shows whatever the opposite of that is. Inconsistency, I guess. Max suffered his first loss of the season which I think the commish believes if probably overdue based on the ink he spilled on it in his last update. But as noted above, this week Max actually had an above average performance. 
...
If you like this view, I would say don't get used to it. This isn't going to be readable by the end of the season. I don't really have a lot to say about this view ever because it is hard to read due to our giant league size. It is crazy to see all those defensive points that Jon put up last week. 
...
Are Jon's WR really that bad? That must be why he keeps asking for Julio Jones. Sorry, Jon, that is one of the players in the NFL who I still recognize the name of, and I have no plans of giving him up. Dave's defense is *still* killing it by keeping pace with both his RB and QB average scores. With his crazy WR scores, he will probably still be in good shape when (if?) the New England defense regresses to the mean. 
...
trans_res <- lapply(1:5, function(m) fromJSON(glue("https://api.sleeper.app/v1/league/", 
                           lg_id, 
                           "/transactions/", m))) 
